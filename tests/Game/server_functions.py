...
class Parser:
    ...
    def check_has_played_old(self,string, websocket, client_id) -> str:
        """
        Input:
        - string
        - websocket
        ...
        if self.played[client_id] >= self.MAX_INPUTS:
            websocket.send_text('Wait Your Turn')
            return ""
        else:
    ...
    def gather_inputs(self,string,websocket,client_id):
...
Use this format: (truth, truth, lie)
    ...
    @staticmethod
    def changeTL(websocket, string, client_id):
        pattern = r"[^,\s][^\,]*[^,\s]*"
        ...
        if len(truths_and_lie) == 3:
            ...
            return websocket.send_text("Invalid format: (truth, truth, lie)")
